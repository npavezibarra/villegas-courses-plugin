        ...
        <script type="text/javascript">
            jQuery(function ($) {
                ...
                $(document.body).on('added_to_cart', function (event, fragments, cart_hash, button) {
                    // Get the product ID from the button that triggered the add to cart
                    var productId = $(button).data('product_id');

                    if (productId) {
                        // Check if product has "Cursos" category via AJAX
                        $.ajax({
                            url: '<?php echo admin_url('admin-ajax.php'); ?>',
                            type: 'POST',
                            data: {
                                action: 'check_product_is_curso',
...
// AJAX handler to check if a product is in "Cursos" category
add_action('wp_ajax_check_product_is_curso', 'villegas_check_product_is_curso');
add_action('wp_ajax_nopriv_check_product_is_curso', 'villegas_check_product_is_curso');

function villegas_check_product_is_curso()
{
    ...
    $is_curso = has_term('Cursos', 'product_cat', $product_id);

    wp_send_json_success(array('is_curso' => $is_curso));
}
...
function non_ajax_redirect_to_checkout($url)
{
    ...
    if (!is_admin()) {
        // Check if a product was just added to cart
        if (isset($_REQUEST['add-to-cart'])) {
            $product_id = intval($_REQUEST['add-to-cart']);

            // Only redirect to checkout if product has "Cursos" category
            if (has_term('Cursos', 'product_cat', $product_id)) {
                $url = wc_get_checkout_url();
            }
            // Otherwise, let WooCommerce use its default behavior (cart page)
        }
    }
    return $url;
}
...
function villegas_inyectar_quiz_data_footer()
{
    if (!is_singular('sfwd-quiz'))
        return; // Asegurarse que estamos en una página de quiz
